...
class Admin_model extends CI_Model {
    ...
    private function _ensurePromoSettingsTable()
    {
    ...
    {
        $this->_ensurePromoSettingsTable();
        return $this->db->where('id', 1)->get('store_promo_settings')->row();
    }

    public function updatePromoTtl($days)
    {
        $days = max(1, min(90, (int)$days));
        $this->_ensurePromoSettingsTable();
        $this->db->where('id', 1)->update('store_promo_settings', ['promo_ttl_days' => $days]);
        return true;
    }

    public function getPromoPool()
    {
        return $this->db->select('id, name, icon, dp, vp, price_type, promo, rate')
                        ->order_by('promo', 'DESC')
                        ->order_by('name', 'ASC')
                        ->get('store_items')->result();
    }

    public function updateItemPromoFlag($id, $promo, $rate)
    {
        $rate = max(1, min(100, (int)$rate));
        $this->db->where('id', (int)$id)->update('store_items', [
            'promo' => $promo ? 1 : 0,
            'rate'  => $rate,
        ]);
        return true;
